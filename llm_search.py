...
5. **취소선(~~) 절대 사용 금지**: "기존: A → 개선: B" 형식
6. **HTML 태그 사용 금지**: <br>, <sup>, <sub> 등 절대 사용 금지. 줄바꿈은 실제 엔터 사용
...
2. 그 다음 용어 설명 (필요시)
3. 그 다음 상세 내용 (배경, 표, 산출근거)
4. 현황/문제점은 참조용, 최종 결과는 개선(안)
5. 표는 원본 그대로 출력하되, 열이 많으면 분리하여 가독성 확보
6. HTML 태그(<br> 등) 절대 사용 금지
7. 여러 연도 문서가 있으면 연도순 정리
8. 각 정보마다 출처 표기: [챕터 | 제목 | 코드 | 날짜]
9. 참조 문서 목록은 출력하지 마세요
"""
# =========================
# 텍스트 전처리 함수
# =========================
